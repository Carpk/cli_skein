class LabyrinthModel
  ...
  def initialize(maze = create_maze)
    @maze = maze
    @position = maze[rand(1..maze.lenth)]
  end

  def move(direction)
    puts @maze[@position[direction.to_sym]] unless @position[direction.to_sym] == nil
  end
  ...
  def create_maze
    { vermillion: {name: "Vermillion Room", north: nil, east: "ochre", south: "aquamarine", west: nil},
      ...
      cobalt: {name: nil, north: "vermillion", east: nil, south: "burnt_sienna", west: "vermillion"},
      violet: {name: nil, north: nil, east: "burnt_sienna", south: "burnt_sienna", west: "chartreuse"},
      burnt_sienna: {name: nil, north: "emerald", east: "lavender", south: nil, west: nil}}
  end
end